/**
    ...
    @FilePath:  src\views\rms\releaseReview\components\addEditDialog\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const addEditDialog = 'releaseReviewAddEditDialog'
import {
...
export const useAddEditDialogStore = defineStore({
  ...
  getters: {
    mapOptions: (state: any) => {
      ...
      const _globalMapOptions:any = state.releaseReviewStore.globalMapOptions;
  ...
  actions: {
    ...
    dataFormat(data :any) {
      ...
      const _globalKeyMap = swapKeysAndValues(this.releaseReviewStore.globalKeyMap)
    ...
    getEditDialogData(data: any) {
      ...
      const _data = data;
      const _id = _data?.id;
      const _globalKeyMap = JSON.parse(JSON.stringify(this.releaseReviewStore.globalKeyMap))
      // 图片字段特殊处理 ，
      // 先删除字段防止 自动赋值报错。
       _globalKeyMap.image = ""
    ...
    getSendData() {
      ...
      const _globalKeyMap = this.releaseReviewStore.globalKeyMap
    ...
    addData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(createDataAPI, _data, '', this.addEditDialogData, this.releaseReviewStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
    editData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(editDataAPI, _data, '', this.addEditDialogData, this.releaseReviewStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
