/**
    ...
    @FilePath:  src\views\rms\release\releaseDetails\components\addEditDialog\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const releaseDetailsStore:any = {}
// addEditDialog 批量创建 防止id重复
const addEditDialog = 'releaseDetailsAddEditDialog'
import {
...
// 命名规范将返回的函数命名为 use releaseDetails Store 是跨可组合项的约定,
// id 命名为 releaseDetails
export const useAddEditDialogStore = defineStore({
  ...
  getters: {
    mapOptions: (state: any) => {
      ...
      const _globalMapOptions:any = state.releaseDetailsStore.globalMapOptions;
  ...
  actions: {
    ...
    dataFormat(data :any) {
      ...
      const _globalKeyMap = swapKeysAndValues(this.releaseDetailsStore.globalKeyMap)
    ...
    getEditDialogData(data: any) {
      ...
      const _data = data;
      const _id = _data?.id;
      const _globalKeyMap = JSON.parse(JSON.stringify(this.releaseDetailsStore.globalKeyMap))
      // 图片字段特殊处理 ，
      // 先删除字段防止 自动赋值报错。
       _globalKeyMap.image = ""
    ...
    getSendData() {
      ...
      const _globalKeyMap = this.releaseDetailsStore.globalKeyMap
    ...
    addData(data: any) {
      console.log("添加",data);
      // releaseDetailsStore.getList()
      // this.addEditDialogData.isShow = false;
      const _data: any = data;
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(createDataAPI, _data, '', this.addEditDialogData, this.releaseDetailsStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
    editData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(editDataAPI, _data, '', this.addEditDialogData, this.releaseDetailsStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
