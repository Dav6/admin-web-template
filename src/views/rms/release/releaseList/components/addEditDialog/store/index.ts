/**
    ...
    @FilePath:  src\views\rms\release\releaseList\components\addEditDialog\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const releaseListStore:any = {}
// addEditDialog 批量创建 防止id重复
const addEditDialog = 'releaseListAddEditDialog'
import {
...
export const useAddEditDialogStore = defineStore({
  ...
  actions: {
    ...
    dataFormat(data :any) {
      ...
      const _globalKeyMap = swapKeysAndValues(this.releaseListStore.globalKeyMap)
    ...
    getEditDialogData(data: any) {
      ...
      const _data = data;
      const _id = _data?.id;
      const _globalKeyMap = JSON.parse(JSON.stringify(this.releaseListStore.globalKeyMap))
      // 图片字段特殊处理 ，
      // 先删除字段防止 自动赋值报错。
       _globalKeyMap.image = ""
    ...
    getSendData() {
      ...
      const _globalKeyMap = this.releaseListStore.globalKeyMap
    ...
    addData(data: any) {
      console.log("添加",data);
      // releaseListStore.getList()
      // this.addEditDialogData.isShow = false;
      const _data: any = data;
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(createDataAPI, _data, '', this.addEditDialogData, this.releaseListStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
    editData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(editDataAPI, _data, '', this.addEditDialogData, this.releaseListStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
