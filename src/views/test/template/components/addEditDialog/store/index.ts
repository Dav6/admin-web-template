/*
  @Author: 戴伟
  @Date: 2023-06-05 08:46:30
  @FilePath: src\views\tms\test\components\addEditDialog\store\index.ts  
  @LastEditTime: ''
  @LastEditors: ''
  @Description: 
        do.....
...
const addEditDialog = 'XXXAddEditDialog'
import {
...
// 命名规范将返回的函数命名为 use XXX Store 是跨可组合项的约定,
// id 命名为 XXX
export const useAddEditDialogStore = defineStore({
  ...
  getters: {
    mapOptions: (state: any) => {
      ...
      const _globalMapOptions:any = state.XXXStore.globalMapOptions;
  ...
  actions: {
    ...
    getEditDialogData(data: any) {
      ...
      const _data = data;
      const _id = _data?.id;
      const _globalKeyMap = JSON.parse(JSON.stringify(this.XXXStore.globalKeyMap))
      // 图片字段特殊处理 ，
      // 先删除字段防止 自动赋值报错。
       _globalKeyMap.image = ""
    ...
    getSendData() {
      ...
      const _globalKeyMap = this.XXXStore.globalKeyMap
    ...
    addData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(createDataAPI, _data, '', this.addEditDialogData, this.XXXStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
    editData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(editDataAPI, _data, '', this.addEditDialogData, this.XXXStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
