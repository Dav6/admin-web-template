/**
    ...
    @FilePath:  src\views\dms\device\deviceVolume\components\container\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const container = 'deviceVolumeContainer'
import {
...
export const useContainerStore = defineStore({
  ...
  getters: {
    listQuery:(state:any)=>{
      ...
      const _listQuery = state.deviceVolumeStore.listQuery;
      return _listQuery;
  ...
  actions: {
    ...
    openDialog(key: string, data: any) {
      ...
      this.deviceVolumeStore.addEditDialogStore.openCloseDialog(true, 'edit', _data)
    ...
    deleteData(data: any, type?: string) {
      ...
      this.APIStore.deleteDataAPI(deleteDataAPI, _ids, _config, this.deviceVolumeStore.getList)
