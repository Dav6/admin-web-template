/**
    ...
    @FilePath:  src\views\dms\device\deviceEdit\components\container\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const container = 'deviceEditContainer'
import {
...
export const useContainerStore = defineStore({
  ...
  getters: {
    listQuery:(state:any)=>{
      ...
      const _listQuery = state.deviceEditStore.listQuery;
      return _listQuery;
  ...
  actions: {
    ...
    openDialog(key: string, data: any) {
      ...
      this.deviceEditStore.addEditDialogStore.openCloseDialog(true, 'edit', _data)
    ...
    deleteData(data: any, type?: string) {
      ...
      this.APIStore.deleteDataAPI(deleteDataAPI, _ids, _config, this.deviceEditStore.getList)
