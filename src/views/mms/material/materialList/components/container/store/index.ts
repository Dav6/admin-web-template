/**
    ...
    @FilePath:  src\views\mms\material\materialList\components\container\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const container = 'materialListContainer'
import {
...
export const useContainerStore = defineStore({
  ...
  getters: {
    listQuery:(state:any)=>{
      ...
      const _listQuery = state.materialListStore.listQuery;
      return _listQuery;
  ...
  actions: {
    ...
    openDialog(key: string, data: any) {
      ...
      this.materialListStore.addEditDialogStore.openCloseDialog(true, 'edit', _data)
    ...
    deleteData(data: any, type?: string) {
      ...
      this.APIStore.deleteDataAPI(deleteDataAPI, _ids, _config, this.materialListStore.getList)
