/**
    ...
    @FilePath:  src\views\pms\program\programList\components\addEditDialog\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const programListStore:any = {}
// addEditDialog 批量创建 防止id重复
const addEditDialog = 'programListAddEditDialog'
import {
...
// 命名规范将返回的函数命名为 use programList Store 是跨可组合项的约定,
// id 命名为 programList
export const useAddEditDialogStore = defineStore({
  ...
  getters: {
    mapOptions: (state: any) => {
      ...
      const _globalMapOptions:any = state.programListStore.globalMapOptions;
  ...
  actions: {
    ...
    dataFormat(data :any) {
      ...
      const _globalKeyMap = swapKeysAndValues(this.programListStore.globalKeyMap)
    ...
    getEditDialogData(data: any) {
      ...
      const _data = data;
      const _id = _data?.id;
      const _globalKeyMap = JSON.parse(JSON.stringify(this.programListStore.globalKeyMap))
      // 图片字段特殊处理 ，
      // 先删除字段防止 自动赋值报错。
       _globalKeyMap.image = ""
    ...
    getSendData() {
      ...
      const _globalKeyMap = this.programListStore.globalKeyMap
    ...
    addData(data: any) {
      console.log("添加",data);
      // programListStore.getList()
      // this.addEditDialogData.isShow = false;
      const _data: any = data;
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(createDataAPI, _data, '', this.addEditDialogData, this.programListStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
    editData(data: any) {
      ...
      //  具体使用 看 src\stores\modules\API\index.ts
      this.APIStore.setDataAPI(editDataAPI, _data, '', this.addEditDialogData, this.programListStore.getList).then((res: any) => {
        // console.log('res',res);
      })
    },
