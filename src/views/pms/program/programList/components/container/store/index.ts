/**
    ...
    @FilePath:  src\views\pms\program\programList\components\container\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const container = 'programListContainer'
import {
...
export const useContainerStore = defineStore({
  ...
  getters: {
    listQuery:(state:any)=>{
      ...
      const _listQuery = state.programListStore.listQuery;
      return _listQuery;
  ...
  actions: {
    ...
    openDialog(key: string, data: any) {
      ...
      this.programListStore.addEditDialogStore.openCloseDialog(true, 'edit', _data)
    ...
    deleteData(data: any, type?: string) {
      ...
      this.APIStore.deleteDataAPI(deleteDataAPI, _ids, _config, this.programListStore.getList)
