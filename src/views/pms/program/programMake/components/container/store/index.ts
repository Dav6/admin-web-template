/**
    ...
    @FilePath:  src\views\pms\program\programMake\components\container\store\index.ts
    @LastEditTime:  ''
    @LastEditors:  ''
    @Description:  do.....
*/
...
const container = 'programMakeContainer'
import {
...
export const useContainerStore = defineStore({
  ...
  getters: {
    listQuery:(state:any)=>{
      ...
      const _listQuery = state.programMakeStore.listQuery;
      return _listQuery;
  ...
  actions: {
    ...
    openDialog(key: string, data: any) {
      ...
      this.programMakeStore.addEditDialogStore.openCloseDialog(true, 'edit', _data)
    ...
    deleteData(data: any, type?: string) {
      ...
      this.APIStore.deleteDataAPI(deleteDataAPI, _ids, _config, this.programMakeStore.getList)
